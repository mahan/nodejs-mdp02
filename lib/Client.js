...
var ZMQReadable = function (_Readable) {
    ...
    function ZMQReadable(zmqClient, options) {
        ...
        var _this = _possibleConstructorReturn(this, (ZMQReadable.__proto__ || Object.getPrototypeOf(ZMQReadable)).call(this, options));
        ...
        function cleanEventsFromZmqClient() {
            zmqClient.removeListener("data", onData);
            zmqClient.removeListener("end", onEnd);
            zmqClient.removeListener("error", onError);
        }

        var onData = function onData(event) {
        ...
        zmqClient.on("data", onData);

        zmqClient.on("end", onEnd);

        zmqClient.on("error", onError);
        return _this;
    }
